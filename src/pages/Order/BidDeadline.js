...
// const dayToCalculate = new Date(today);

export default function BidDeadline() {
  const [daysLeft, setDaysLeft] = useState(30);

  // const calculateDaysLeft = day => {
  //   dayToCalculate.setDate(day.getDate() + daysLeft);
  //   return dayToCalculate.toLocaleDateString();
  // };
  ...
  return (
    <Container>
      <Title>입찰 마감기한</Title>
      <Deadline>
        {/* {daysLeft}일 ({calculateDaysLeft(today)} 마감) */}
        {daysLeft}일
      </Deadline>
      <DaysLeftWrap>
        {daysList.map(day => {
